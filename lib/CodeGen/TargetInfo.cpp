//===---- TargetInfo.cpp - Encapsulate target details -----------*-
// C++ -*-===//
//
...
bool ABIInfo::shouldSignExtUnsignedType(QualType Ty) const { return false; }
...
public:
  PPC64_SVR4_ABIInfo(CodeGen::CodeGenTypes &CGT, ABIKind Kind, bool HasQPX,
                     bool SoftFloatABI)
      : ABIInfo(CGT), Kind(Kind), HasQPX(HasQPX), IsSoftFloatABI(SoftFloatABI) {
  }
...
bool ABIInfo::isHomogeneousAggregate(QualType Ty, const Type *&Base,
  ...
  if (const ConstantArrayType *AT = getContext().getAsConstantArrayType(Ty)) {
    ...
    for (const auto *FD : RD->fields()) {
      ...
      if (getContext().getLangOpts().CPlusPlus && FD->isBitField() &&
          FD->getBitWidthValue(getContext()) == 0)
        continue;
...
ABIArgInfo PPC64_SVR4_ABIInfo::classifyArgumentType(QualType Ty) const {
  ...
  return (isPromotableTypeForABI(Ty) ? ABIArgInfo::getExtend()
                                     : ABIArgInfo::getDirect());
}
...
ABIArgInfo PPC64_SVR4_ABIInfo::classifyReturnType(QualType RetTy) const {
  ...
  return (isPromotableTypeForABI(RetTy) ? ABIArgInfo::getExtend()
                                        : ABIArgInfo::getDirect());
}
...
ABIArgInfo SystemZABIInfo::classifyReturnType(QualType RetTy) const {
  ...
  if (isCompoundType(RetTy) || getContext().getTypeSize(RetTy) > 64)
    return getNaturalAlignIndirect(RetTy);
  return (isPromotableIntegerType(RetTy) ? ABIArgInfo::getExtend()
                                         : ABIArgInfo::getDirect());
}
...
void RISCVABIInfo::computeInfo(CGFunctionInfo &FI) const {
  ...
  // be passed indirect or if the type size is greater than 2*xlen. e.g.
  // fp128
...
ABIArgInfo RISCVABIInfo::extendType(QualType Ty) const {
  ...
  // TODO
  //  if (XLen == 64 && Ty->isUnsignedIntegerOrEnumerationType() &&
  //           TySize == 32)
...
//  return DefaultABIInfo::EmitVAArg(CGF, VAListAddr, Ty);
